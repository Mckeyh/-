...
**实测结果（本机 RTX 8GB，36 类，每类上限 200 张 = 6685 张）**：

| 训练配置 | 内部留出集 1337 张 Top-1 / Top-5 |
|---|---|
| `--epochs 10`（只训分类头，主干冻结） | 32.76% / 65.45% |
| `--epochs 6 --unfreeze`（解冻 layer4） | 32.9% / — |
| **`--epochs 8 --unfreeze-all`（全主干微调，fp32）** | **38.07% / 73.15%** |

**最终交付的模型（上表最后一行）在两套测试集上的成绩**：

| 测试集 | 张数 | Top-1 | Top-5 |
|---|---|---|---|
| 内部留出集（训练集随机 20%，种子 42） | 1337 | 38.07% | 73.15% |
| **IP102 官方 val 划分（训练完全没见过，更严格）** | 2582 | **33.38%** | **69.02%** |

> 官方 val 测试集由 `backend/data/upload/dataset/val_holdout_ip102/` 提供：
> 从 IP102 官方 `validation` 分片抽出本项目这 36 类的图片，**不参与任何训练与调参**。

**微调踩过的坑（很值得写进答辩：调参不是玄学，是逐条定位问题）**：
...
5. **"数据越多越好"在这个数据集上不成立**：把每类上限从 200 放开到全量（6685 → 15495 张，
   蚜虫一类就有 2456 张、cerodonta 只有 82 张，31 倍不均衡）后，同一套留出集上准确率反而
   从 36.5% 降到 31~35%。原因是多出来的样本以"难样本/弱标注"为主，且极端不均衡让高频类主导梯度。
   因此最终仍采用 **每类 200 张** 的子集方案。
6. **采样策略必须与测试集分布一致**：`WeightedRandomSampler` 会把训练分布拉成"均匀"，
   而留出集/官方 val 都是"自然分布"，两者错配会直接掉点（实测 val 从 34.6% 掉到 29%）。
   现在按「不均衡度 ≥ 4 倍」自动改用自然分布采样。
7. **改类别目录名 = 改类别编号**：`ImageFolder` 按**目录名字符串排序**分配类别号。
   把第 17 类文件夹从 `white margined moth` 改成 `白边地老虎` 后，类别顺序整体位移，
   若不同步更新 `class_names.json`，预测出的中文名会整体串位（评估时一度出现"模型明明没变、
   准确率却掉 7 个点"的假象）。**改名前务必确认 `sorted(目录名) == class_names.json`**。
8. **TTA（水平翻转取平均）收益在噪声级别**：内部留出集 Top-1 −0.22/Top-5 +0.75，
   官方 val Top-1 +0.43/Top-5 −0.27，却让推理耗时翻倍 —— 故 `TTA_ENABLED` 默认关闭。

**下一步还能怎么提升**（时间换准确率）：每类 200 张这个子集上 train_acc 到第 8 轮仍在上升，
把 cosine 调度放到 12~15 epoch、并用**官方 val 划分**做模型选择、再下官方 `test` 分片做最终测试，
预计还能再涨若干个百分点；更根本的是换更大主干（ResNet101 / ConvNeXt）或按"近缘种合并"重定义类别。
